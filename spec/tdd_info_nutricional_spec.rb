...
RSpec.describe Tdd_valor_nutricional do 
  before:all do
	...
	@persona6 = Persona_datos_antro.new(97,1.80,20,22,34,"18:30","Sí","Sí",[11, 14 ,12],[26, 27, 28],[10, 7, 5],[5, 4, 2],"20","Hombre    ",0.27)
      @persona7 = Persona_datos_antro.new(124,1.86,33,57,88,"18:45","Sí","No",[33, 22 ,23],[26, 30, 24],[9, 7, 6],[10, 4, 6],"24","Hombr    e",0.12)
      @persona8 = Persona_datos_antro.new(130,1.77,20,25,36,"19:00","No","Sí",[29, 40 ,15],[30, 15, 24],[20, 10, 6],[9, 5, 2],"29","Muje    r",0.0)
      @persona9 = Persona_datos_antro.new(102,1.82,20,25,36,"19:00","No","Sí",[29, 40 ,15],[30, 15, 24],[20, 10, 6],[9, 5, 2],"29","Muje        r",0.0)
      @persona10 = Persona_datos_antro.new(112,1.84,20,25,36,"19:00","No","Sí",[29, 40 ,15],[30, 15, 24],[20, 10, 6],[9, 5, 2],"29","Muje            r",0.0)
  ...
  describe "prueba10"do
   ...
   describe"Pruebas prct11"do
	...
	expect(@vec_menus.sort).to eq([@menu8,@menu3,@menu2,@menu7,@menu9,@menu6,@menu4,@menu1,@menu5,@menu10])
     ...
     it"lista pacientes"do
	@lista_pacientes2.insert_tail(@paciente1)
	expect(@lista_pacientes2.tail.value).to eq(@paciente1)
     end
     
     it"2"do
	@lista_pacientes2.insert_tail(@paciente2)
	expect(@lista_pacientes2.tail.value).to eq(@paciente2)
     end
     it"3"do
	@lista_pacientes2.insert_tail(@paciente3)
	expect(@lista_pacientes2.tail.value).to eq(@paciente3)
     end
